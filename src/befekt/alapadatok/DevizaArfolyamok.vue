    ...
    >
        <template v-slot:top>
            <v-toolbar 
                ...
                color="yellow"
            >
                <v-toolbar-title>Devizaárfolyamok karbantartása</v-toolbar-title>
                <v-divider
                    class="mx-4"
                    inset
                    vertical
        ...
        <template v-slot:item.action="{ item }">
            ...
            <v-tooltip bottom>
                ...
                <span>Bevétel havi könyvelése, ill. annak törlése</span>
            </v-tooltip>
        </template>
    </v-data-table>
</template>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            haviKonyvItem: '',                              // Ha havi könyvelésről van szó, a könyvelendő tétel
        ...
        methods: {
            ...
            changeBefektFajta(newBefektFajta) {             // Befektetés fajta változásakor hajtódik végre
                if (this.kivBefekt != newBefektFajta) {
                    ...
                    this.osszesBevetelFormUpdate(this.kivBefekt);
                }
            },
            ...
            igenValasz() {                                  // Confirm panelen: Új tétel fölvétele/Régi módosítása
                ...
                } else if (this.confirmMode === 'H') {
                    this.havikonyvBevetel();
                } else if (this.confirmMode === 'I') {
                    this.havikonyvTorlesBevetel();
                }
            },
            ...
            haviKonyveles(item) {                           // Havikönyvelés végrehajtása
                this.haviKonyvItem = item
                if (this.haviKonyvItem.bevHavKonyv) {
                    this.confirmMode = 'I'
                    this.confirmOpen = true
                } else {
                    if (!item.bevSzlaKonyv) {
                        this.uzenetText = Util.nemHavikonyv;
                        this.alert = true;
                    } else {
                        this.confirmMode = 'H'
                        this.confirmOpen = true
            ...
            async havikonyvBevetel() {                      // Havi könyvelés végrehajtása
                try {
                    const adat = await this.$axios.delete(
                        'http://localhost:8080/bevetels/havikonyveles/' + this.szlaKonyvItem.id + '/' + this.szlaKonyvItem.bevMddat);
                    if (adat.status === 208) {
                        this.uzenetText = adat.data;
                        this.alert = true;
                    } else {
            ...
            async havikonyvTorlesBevetel() {                // Havi könyvelés törlésének végrehajtása
                try {
                    const adat = await this.$axios.delete(
                        'http://localhost:8080/bevetels/havikonytorles/' + this.szlaKonyvItem.id + '/' + this.szlaKonyvItem.bevMddat);
                    if (adat.status === 208) {
                        this.uzenetText = adat.data;
                        this.alert = true;
                    } else {
